...
writes two ready-to-use tokens (reader and publisher) to fake_idp.env.
...
if __name__ == "__main__":
    env_path = Path("fake_idp.env")
    env_path.write_text(
        f"export READER_JWT={mint('reader@example.invalid', ['reader'])}\n"
        f"export PUBLISHER_JWT={mint('publisher@example.invalid', ['reader', 'publisher'])}\n"
    )
    print()
