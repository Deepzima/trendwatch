...
check("only hacker news is fetched live", "hn.algolia.com" in tsrc)
check("live fetch is a single Algolia search, not per-item Firebase GETs",
      "hacker-news.firebaseio.com" not in tsrc)
...
# Default request-guard scope is systemPrompt + messages. This payload arrives
# as a tool result, so the guard is a no-op unless toolOutput is in scope.
check("request guard scope includes toolOutput", "toolOutput" in req_block)
